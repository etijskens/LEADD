...
EvolutionReport::EvolutionReport(const std::string& output_directory_path) {
  ...
  std::experimental::filesystem::path path (output_directory_path);
  std::experimental::filesystem::path scores_csv_path = path;
  std::experimental::filesystem::path n_ring_atoms_csv_path = path;
  std::experimental::filesystem::path operation_frequencies_csv_path = path;
...
void EvolutionReport::WriteWeights(const ReconstructedMol& reconstruction, const std::string& file_name) {
  ...
  std::experimental::filesystem::path weights_csv_path = directory;
...
bool ReconstructedMol::Substitution(const std::string & location, sqlite3_stmt * select_statement, ConnectionQueryResults & query_results, SizeController & controller, std::mt19937 & prng, bool guided, bool update_stereo) {
  ...
  if (location == "peripheral") {
    ...
    throw std::runtime_error(std::string("Invalid location option for Substitution."));
  };
...
float ReconstructedMol::GetSimilarity(ReconstructedMol& reconstruction) {
  GenerateFingerprint();
