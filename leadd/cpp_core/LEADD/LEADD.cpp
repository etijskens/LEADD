...
LEADD::LEADD(const std::string& settings_file_path, const std::string& output_directory_path) {
  // Read the settings file and verify that the settings are valid.
  try {
    settings = ReconstructionSettings(settings_file_path);
    settings.CheckRequiredInputFiles();
  ...
  // If the SAScore filter is enabled, deserialize the FeatureLibrary.
...
void LEADD::LoadPopulation() {
  ...
  archive >> population;
  input_stream.close();
  // Transfer the MolBrick ownership to the corresponding ReconstructedMols.
  MakePopulationTakeBrickOwnership();
  // If necessary, assign fresh weights to the population's ConnectionPoints.
  if (settings.UsingGuidedEvolution() && settings.ResetWeightsOnRestart()) {
    AssignConnectorWeightsToPopulation();
  };
  // Reassign the maximum ID.
  for (const ReconstructedMol& individual : population) {
    unsigned id = individual.GetID();
...
unsigned LEADD::ScoreChildrenExternally() {
  ...
  assert(result == 0);
  std::experimental::filesystem::path scores_path(settings.GetScoresOutputFile());
  if (!std::experimental::filesystem::exists(scores_path)) {
