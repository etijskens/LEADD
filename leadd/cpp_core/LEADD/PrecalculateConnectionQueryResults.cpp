...
int main(int argc, const char* argv[]) {
  ...
  std::experimental::filesystem::path input_path(input);
  ...
  if (!settings_file.empty()) {
    std::experimental::filesystem::path settings_path(settings_file);
    if (!std::experimental::filesystem::exists(settings_path)) {
      throw boost::program_options::validation_error(boost::program_options::validation_error::invalid_option_value);
    } else {
