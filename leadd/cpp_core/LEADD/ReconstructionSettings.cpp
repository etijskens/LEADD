...
ReconstructionSettings::ReconstructionSettings(const std::string& settings_file) {
...
float ReconstructionSettings::GetMaxSAScore() const {
  return max_sascore;
};
...
bool FileIsAccessible(const std::string& path_str) {
  std::experimental::filesystem::file_status status = std::experimental::filesystem::status(path_str);
  if (std::experimental::filesystem::exists(status)) {
    return true;
  };
  return false;
};
...
bool FileRootDirectoryIsAccessible(const std::string& path_str) {
  std::experimental::filesystem::path path(path_str);
